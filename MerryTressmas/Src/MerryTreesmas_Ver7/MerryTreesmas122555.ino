...
uint8_t food_current = 0;//메뉴 상태
uint8_t menu_current = 0;//메뉴 상태

unsigned long healthCount = 0;  //mills함수와 연동
...
void drawMenu(void) {           //첫번째 메뉴를 그리는 함수
  uint8_t i, h;
  u8g_uint_t w, d;
...
void drawFoodMenu(void) {           //세번째 화면, 두번째 메뉴를 그리는 함수, 음식고르는 메뉴
  ...
  u8g.setFont(u8g_font_5x7);//4x6 5x7 5x8 6x10 6x12 6x13
  u8g.setFontRefHeightText();
  u8g.setFontPosTop();
...
void escape(void) {             //키를 입력받았을때 왼쪽 버튼을 누르면 탈출 함수(메뉴 부분에만 적용)
  switch( uiKeyCode ){
  case KEY_BACK:  //게임안에서 뒤메뉴로
    action=0;
    break;
  case KEY_NONE: //게임안에서 먹이메뉴로
  if(menu_current==2)
    action=2;
    break;
  }
...
void Tree_state(){                    //빈이 자라기 전 상태출력함수
  ...
  u8g.drawBox(30,2, health ,6);
  if(  millis() - healthCount >1000)
  {
    healthCount = millis();
    if(DHT.temperature>27 || DHT.temperature<25){
      if(health>0)
        health--;
      if(health==0)
        bin_death=ture;
