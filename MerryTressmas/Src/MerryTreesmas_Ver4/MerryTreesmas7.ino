...
void TreesMas(){
  u8g.drawBitmapP( 40, 12, 5, 55, bitmap_Tree);
  delay(100); // 약 5~6초
}
void logo1(){
...
void setup() {
  ...
  Serial.begin(9600);
  pinMode(inputPin, INPUT);     // 마그네틱 센서를 인풋으로 설정해놓음(디지털값)
  switch (chk){
  case DHTLIB_OK:  
    Serial.print("OK,\t"); 
    break;
...
void loop() {  
  ...
  val = digitalRead(inputPin);      //마그네틱의 값을 받음
                                    // 마그네틱이 작동 됬다면
  if((val==HIGH&&(25<=DHT.temperature&&DHT.temperature<=27))||the_end){
    ...
    if(isEnd == false){ 
      ...
      u8g.firstPage();
      do  {
        u8g.drawBitmapP( x, y, 4, 25, bitmap_Sun);  //원하는 그림을 조작 가능
      } while( u8g.nextPage() );
    }
  }
